                                            ...
                                            const QFileInfo &sourceInfo)
{
    ...
    QString candidate = destinationDir.filePath(originalName);
    ...
    for (int i = 1; ; ++i)
    {
        ...
        candidate = destinationDir.filePath(fileName);
        if (!QFileInfo::exists(candidate))
            return candidate;
    }
}
...
bool PlaylistController::curateCurrentToBucket(int bucket)
{
    ...
    {
        vlc_playlist_locker locker(d->m_playlist);
        const ssize_t currentIndex = vlc_playlist_GetCurrentIndex(d->m_playlist);
        if (currentIndex < 0)
            return false;

        vlc_playlist_item_t * const playlistItem = vlc_playlist_Get(d->m_playlist, currentIndex);
        if (!playlistItem)
            return false;

        item = PlaylistItem(playlistItem);
        input_item_t * const input = vlc_playlist_item_GetMedia(playlistItem);
        if (!input)
            return false;

        auto uri = vlc::wrap_cptr(input_item_GetURI(input));
        if (!uri || uri.get()[0] == '\0')
            return false;

        auto path = vlc::wrap_cptr(vlc_uri2path(uri.get()));
        if (!path || path.get()[0] == '\0')
            return false;

        sourcePath = QString::fromUtf8(path.get());
    }

    QFileInfo sourceInfo(sourcePath);
    if (!sourceInfo.exists() || !sourceInfo.isFile() || sourceInfo.isHidden())
        return false;

    QDir sourceDir = sourceInfo.absoluteDir();
    const QString bucketName = QString::number(bucket);
    if (!sourceDir.mkpath(bucketName))
        return false;

    QDir destinationDir(sourceDir.filePath(bucketName));
    const QString destinationPath = nextAvailableDestinationPath(destinationDir, sourceInfo);
    if (!QFile::rename(sourceInfo.absoluteFilePath(), destinationPath))
        return false;

    vlc_playlist_locker locker(d->m_playlist);
    const ssize_t itemIndex = vlc_playlist_IndexOf(d->m_playlist, item.raw());
    if (itemIndex < 0)
        return true;

    vlc_playlist_item_t *rawItem = item.raw();
    int ret = vlc_playlist_RequestRemove(d->m_playlist, &rawItem, 1, itemIndex);
    if (ret != VLC_SUCCESS)
    {
        QFile::rename(destinationPath, sourceInfo.absoluteFilePath());
        return false;
    }

    const size_t remainingCount = vlc_playlist_Count(d->m_playlist);
    if (static_cast<size_t>(itemIndex) < remainingCount)
    {
        ret = vlc_playlist_GoTo(d->m_playlist, itemIndex);
        if (ret == VLC_SUCCESS)
            vlc_playlist_Start(d->m_playlist);
    }
    else
