...
#undef KeyPress
#endif
void MainCtx::sendHotkey(Qt::Key key, Qt::KeyboardModifiers modifiers, bool autoRepeat)
{
    if (!autoRepeat && key >= Qt::Key_0 && key <= Qt::Key_9)
    {
        if (p_intf->p_mainPlaylistController)
            p_intf->p_mainPlaylistController->curateCurrentToBucket(key - Qt::Key_0);
        return;
    }
